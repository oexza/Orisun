...
- FoundationDB storage backend (`ORISUN_BACKEND=foundationdb`, built with `-tags foundationdb`). Event positions come from FoundationDB commit versionstamps, so plain appends and commands on different aggregates commit in parallel — no per-boundary sequence counter on the write path. Consistency conditions are checked through ready covering indexes with conflict ranges scoped to the matching index slice, and fail closed (`FAILED_PRECONDITION`) when no covering index exists.
