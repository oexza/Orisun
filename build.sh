...
GOOS=$TARGET_OS GOARCH=$TARGET_ARCH go build -tags development="false" -a -installsuffix cgo \
  -ldflags="-w -s -X 'github.com/oexza/orisun/common.Version=$VERSION' -X 'github.com/oexza/orisun/common.BuildTime=$(date -u +'%Y-%m-%dT%H:%M:%SZ')' -X 'github.com/oexza/orisun/common.GitCommit=$(git rev-parse --short HEAD)'" \
