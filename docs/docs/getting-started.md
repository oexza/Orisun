...
You can run Orisun as a release binary, a Docker image, an embedded Go package, an on-device SQLite library for Android and iOS, or an embedded native Flutter library. Docker Compose is convenient for trying the full stack, but production deployments can run the binary directly under systemd, Nomad, Kubernetes, Fly, Render, or any process supervisor.

This guide starts a standalone server. If you want Orisun inside a Go process, go to [Go Embedding](./embedding/go). For a native Android or iOS application with no server, gRPC, or NATS runtime, use [Mobile Embedding](./embedding/mobile). Flutter applications on Android, iOS, macOS, Windows, and Linux can use [Flutter Embedding](./embedding/flutter).
...
| Flutter native package | You want typed Dart access to embedded SQLite and CCC. | [Flutter Embedding](./embedding/flutter) |
...
Orisun starts embedded NATS JetStream by default in standalone binaries, containers, and the standard embedded Go packages. The mobile and Flutter SQLite runtimes are deliberately NATS-free; their subscriptions catch up directly from SQLite after in-process commit notifications, with polling as a no-miss fallback.
