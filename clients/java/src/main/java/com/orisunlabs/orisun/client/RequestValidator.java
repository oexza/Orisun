...
public class RequestValidator {
    ...
    public static void validateSaveEventsRequest(Eventstore.SaveEventsRequest request) {
        ...
        if (request.getStream() == null) {
            throw new OrisunException("Stream information is required")
                .addContext("operation", "saveEvents")
                .addContext("boundary", request.getBoundary());
        }
        
        if (request.getStream().getName() == null || request.getStream().getName().trim().isEmpty()) {
            throw new OrisunException("Stream name is required")
                .addContext("operation", "saveEvents")
                .addContext("boundary", request.getBoundary());
        }
        ...
        if (request.getEventsCount() == 0) {
            throw new OrisunException("At least one event is required")
                .addContext("operation", "saveEvents")
                .addContext("streamName", request.getStream().getName())
                .addContext("boundary", request.getBoundary());
        }
        ...
        for (int i = 0; i < request.getEventsCount(); i++) {
            ...
            validateEventToSave(event, i, request.getStream().getName(), request.getBoundary());
        }
    }
     ...
     * @param streamName The stream name for error context
     * @param boundary The boundary for error context
     * @throws OrisunException if validation fails
     */
    private static void validateEventToSave(Eventstore.EventToSave event, int index, String streamName, String boundary) {
        if (event.getEventId() == null || event.getEventId().trim().isEmpty()) {
            throw new OrisunException("Event at index " + index + " is missing eventId")
                .addContext("operation", "saveEvents")
                .addContext("eventIndex", index)
                .addContext("streamName", streamName)
    ...
    public static void validateGetEventsRequest(Eventstore.GetEventsRequest request) {
        ...
        // Validate stream if specified
        if (request.hasStream() && request.getStream().getName() != null && 
            request.getStream().getName().trim().isEmpty()) {
            throw new OrisunException("Stream name cannot be empty when stream is specified")
                .addContext("operation", "getEvents")
                .addContext("boundary", request.getBoundary());
        }
    }
    ...
    /**
     * Validate a CatchUpSubscribeToStreamRequest
     * @param request The request to validate
     * @throws OrisunException if validation fails
     */
    public static void validateSubscribeToStreamRequest(Eventstore.CatchUpSubscribeToStreamRequest request) {
        if (request == null) {
            throw new OrisunException("SubscribeToStreamRequest cannot be null")
                .addContext("operation", "subscribeToStream");
        }
        ...
        if (request.getBoundary() == null || request.getBoundary().trim().isEmpty()) {
            throw new OrisunException("Boundary is required")
                .addContext("operation", "subscribeToStream")
                .addContext("request", "CatchUpSubscribeToStreamRequest");
        }
        ...
        if (request.getStream() == null || request.getStream().trim().isEmpty()) {
            throw new OrisunException("Stream name is required")
                .addContext("operation", "subscribeToStream")
                .addContext("boundary", request.getBoundary());
        }
